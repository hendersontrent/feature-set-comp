...
# function in theft but with edits
# that tracks computation time instead
#
# NOTE: setup.R must be run first
#------------------------------------

#----------------------------------------
# Author: Trent Henderson, 19 August 2021
#----------------------------------------

#------------------- Helper functions for comp times ---------------

calc_comp_time <- function(feature_set = NULL){
  
  if("catch22" %in% feature_set){
    m <- data.frame(summary(microbenchmark(calc_catch222(tmp), times = 1, unit = "s"))) %>%
  ...
  if("feasts" %in% feature_set){
    m <- data.frame(summary(microbenchmark(calc_feasts2(tmp), times = 1, unit = "s"))) %>%
      dplyr::select(c(mean, min, max))
  ...
  if("tsfeatures" %in% feature_set){
    m <- data.frame(summary(microbenchmark(calc_tsfeatures2(tmp), times = 1, unit = "s"))) %>%
      dplyr::select(c(mean, min, max))
  ...
  if("tsfresh" %in% feature_set){
    m <- data.frame(summary(microbenchmark(calc_tsfresh2(tmp), times = 1, unit = "s"))) %>%
      dplyr::select(c(mean, min, max))
  ...
  if("tsfel" %in% feature_set){
    m <- data.frame(summary(microbenchmark(calc_tsfel2(tmp), times = 1, unit = "s"))) %>%
      dplyr::select(c(mean, min, max))
  ...
  if("kats" %in% feature_set){
    m <- data.frame(summary(microbenchmark(calc_kats2(tmp), times = 1, unit = "s"))) %>%
      dplyr::select(c(mean, min, max))
...
calc_tsfeatures2 <- function(data){
...
calc_tsfresh2 <- function(data, column_id = "id", column_sort = "timepoint", cleanup){
...
calculate_features2 <- function(data, id_var = NULL, time_var = NULL, values_var = NULL, group_var = NULL,
  ...
  if("catch22" %in% feature_set){
    ...
    tmp <- calc_catch222(data = data_re)
  }
  
  if("feasts" %in% feature_set){
    
    tmp1 <- calc_feasts2(data = data_re)
  }
  
  if("tsfeatures" %in% feature_set){
    
    tmp2 <- calc_tsfeatures2(data = data_re)
  }
  ...
  if("tsfresh" %in% feature_set){
    ...
    tmp3 <- calc_tsfresh2(data = data_re, column_id = "id", column_sort = "timepoint", cleanup = cleanuper)
  }
